...
{
    public class ResourceListDebugUIElement : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                foreach (string type in WorldInfo.ResourceTypes)
                {
                    AddEntry(type);
                }
            }
        ...
        {
            var entry = Instantiate(_entryPrefab, transform).GetComponent<ResourceListDebugUIElementEntry>();
            entry.Type = type;
