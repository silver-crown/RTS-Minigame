...
//                        who.affectedByProgress.add(new StepNeuro<StepV>(whom, -1, 0.2f)); //affect others
        ...
        public static void fillNeuroNPares2(List<Step2> steps, float affectPare) {
            cleanupNeuro(steps);
            float affectSelfC = 2;
            float affectPareC = affectPare / steps.Count;
            float affectCounterC = 20f / steps.Count;
            float rewardCounterC = 8f / steps.Count;
            
            foreach (Step2 who in steps) {
                who.affectedByDeviation.Add(new StepNeuro<Step2>(who, 0, affectSelfC, "self"));
            }
            for (int i = 0; i < steps.Count; i++) {
                ...
                for (int j = 0; j < steps.Count; j++) {
                    ...
                    who.affectedByDeviation.Add(new StepNeuro<Step2>(whom, 0f, rewardCounterC, "devFrom" + i));
